...
4. **Settled ≠ ready.** `curl -X POST localhost:4800/ctl -d '{"renderDelayMs":900}'` then
   ...
   `settled:network` while `eval` still says "loading…". Let that late render land before the next click
   (`disco watch --fn "() => document.querySelector('#chart-status').textContent === 'idle'"`), then the
   same click with the postcondition on it:
   `disco act click '#load-chart' --until-fn "() => document.querySelector('#chart-status').textContent === 'idle'"` —
   `✓ until: matched in ≈ slowMs + renderDelayMs` (~1300ms at the default `slowMs` 400), and `eval`/post-state
   read "idle". Reset: `curl -X POST localhost:4800/ctl -d '{"renderDelayMs":0}'`.
...
8. `disco act dblclick '#dbl-target'` / `disco act drag '#slider-thumb' --to-dx 150 --to-dy 0` /
   `disco act rightclick '#ctx-target'` — pointer grammar beyond clicks. The context menu stays open:
   `disco act press Escape` (or click an item) before the next step — a click elsewhere would come back
   `diagnosis: occluded by <ul role="menu" id="ctx-menu">`, which is the right answer.
9. `disco act click 'role=button[name="Load Rows"]' --until-url /api/rows --until-landed --eval "function(){ return document.querySelectorAll('#rows .row').length }"`
   ...
   `disco sql gauntlet "SELECT length(b.text) FROM requests r JOIN bodies b ON b.hash=r.body_hash WHERE r.path='/api/rows' AND r.run=(SELECT max(run) FROM runs)"`

...
- **login** (idempotent; handles a location/role/department picker if one exists — present OR absent;
  detects the app's own failure message; reaches the shell anchor; on a cold load verify the fields hold
  what you typed — SPA hydration races swallow the first keystrokes, and the first check run *failing* on
  that is the point: the diagnosis is the bug report)

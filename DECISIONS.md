...
- **Exam C (orangehrm, never touched: 18 min / 12 checks / 4 verified writes / 11 friction) fold-back:** a until cut off by `max` names its code; `json` waits its 1 s for a matching request whose headers are still in flight; `look` falls back to an anchored, whitespace-tolerant role regex before a css path; one README sentence names the re-read-the-server pattern. Rejected: a redirect the app fires 3 s after "quiet" (no verdict — the `nav` table has it).
- **Exam C (openmrs with writes: 20 min / 9 checks incl. 7 verified writes / 0 s expired / 8 friction) fold-back:** `look` names controls as the accessibility tree does (aria-hidden icons skipped, a label beats a placeholder; a name that came only from a placeholder is re-read from Playwright's aria view); off-canvas means left of, above, or right of the page — never below the fold; `sql --json` has one shape: an array of rows with JSON text cells parsed. Rejected: a read that the app performs with POST showing under `writes` (the line is a tripwire, not a verdict).

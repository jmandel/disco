...
(a JPEG path: view it), open `dialogs`. The selector is durable: `data-test`, a stable id, a unique `role=…[name="…"]`,
else a short css path. With a selector or `Locator`: `count`, `matches` (`{ n, selector, tag, role, name, text, box,

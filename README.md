...
apps/<app>/
  ...
  sdk/         the same sdk as a folder once one file stops being readable — index.ts is the entry, the split is the product's own
...
report is in `evidence/`; a number without an act id is a guess. Write the README from the evidence files, not from
...
that put that screen there: its evidence carries the accessibility tree it left behind. Workflow narrative — precondition, steps,
postcondition, side effects, gotchas — lives in the docblock above the function, once.

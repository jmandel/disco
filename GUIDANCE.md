...
- **Ambient traffic profile:** let the page sit untouched for the first ~30–60 seconds — and **longer for EHRs**, whose keepalive/reminder heartbeats are often on ~60-second periods, so the classifier needs ≥3 cycles (~2 minutes) of idle to catch them (dogfood #1: OpenEMR's `dated_reminders*` + `background_service/$run` trio). A too-short warm-up leaves a heartbeat unclassified, and it then holds settlement open on every action. (`disco session new` does this by default; `disco idle [ms]` re-runs it), then read the classifier's families (the `families` table; `disco families`): heartbeats, polls, token refresh, analytics. This is not just documentation — the classifier in §4.4 has no evidence until this happens, and acting before it does yields noisy settlement. Note session-keepalive behavior and idle-timeout policy — discovery sessions are long, and knowing the timeout (and how the warning presents) prevents mysterious mid-session death.
...
- **Disabled controls:** a `disabled` / `aria-disabled` control is typically `pointer-events:none`, so a hit-test resolves to an *ancestor* — disco reports `not-interactable` with the interactability facts (never "occluded by the element's own parent"). A disabled submit is the app telling you a precondition is unmet (a field not yet dirty-validated, a required selection missing); find and satisfy the precondition, don't retry the click or try to dismiss an "overlay".

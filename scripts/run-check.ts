...
if (!app) { console.error("usage: node scripts/run-check.ts <app> [--headed] [--close] [--bail]"); process.exit(2); }
...
const step: Step = async (name, fn) => {
  if (bailed) { results.push({ name, ok: false, ms: 0, error: "skipped (--bail)" }); console.log(`SKIP ${name}`); return false; }
  ...
  catch (e) { const error = String((e as Error).message ?? e).split("\n")[0]; results.push({ name, ok: false, ms: Math.round(performance.now() - t), error }); console.log(`FAIL ${name} (${Math.round(performance.now() - t)}ms): ${error}`); if (bail) bailed = true; return false; }
};
